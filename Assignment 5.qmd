...
mod.dxa %>%
 ggplot(aes(time, lean.mass, group = paste(participant, group), color = group)) + geom_line() + theme_bw() +
  labs(title = "Table 2: Change in lean body mass",
       ...
       x = "Time",
       caption = "Table 2: Shows the difference between multiple and single sets for each participant, sorted by colour.")
```
## Muskelstyrke
...
str.dat %>%    
  ...
  gt() %>% 
  tab_header(
    title = "Table 3: Average change in strength from 3 and 1 sets",
...
str.mod %>%
 ...
 ggplot(aes(time, combined.load, group = paste(participant, sets), color = sets)) + geom_line() + theme_bw() + labs(title = "Table 4: Change in strength",
       y = "Change in strength (combined load)",
       x = "Time",
       caption = "Table 4: Shows the difference between multiple and single sets on strength change.")
```
